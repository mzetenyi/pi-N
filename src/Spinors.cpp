...
namespace Spinors {
  ...
  /// Adjoint
  const AdPauliSpinor PauliSpinor::adj() const {
    return AdPauliSpinor(conj(x1),conj(x2));
  }
  const AdPauliSpinor adj(const PauliSpinor& s) {
    return AdPauliSpinor(conj(s.x1),conj(s.x2));
  }
  /// Positive sign.
  const PauliSpinor& PauliSpinor::operator + () const {
    return *this;
  }
  /// Negative sign.
  const PauliSpinor PauliSpinor::operator - () const {
    return PauliSpinor(-x1,-x2);
  }
  /// Spinor addition.
  const PauliSpinor PauliSpinor::operator + (const PauliSpinor& s) const {
  ...
  const PauliSpinor PauliSpinor::operator * (const dcomplex& z) const {
  ...
  const PauliSpinor operator * (const dcomplex& z, const PauliSpinor& s) {
    return PauliSpinor(z*s.x1, z*s.x2);
  }
  /// Matrix*spinor product.
  const PauliSpinor operator * (const PauliMatrix& tau, const PauliSpinor& s) {
    return PauliSpinor(tau.x11*s.x1 + tau.x12*s.x2, tau.x21*s.x1 + tau.x22*s.x2);
  }
  /// Scalar product.
  const dcomplex operator * (const AdPauliSpinor& a, const PauliSpinor& s) {
    return a.x1*s.x1 + a.x2*s.x2;
  }
  /// Diadic product.
  const PauliMatrix operator * (const PauliSpinor& s, const AdPauliSpinor& a) {
  ...
  */
  ...
  bool PauliSpinor::isnan() const {
    return ::isnan(x1) or ::isnan(x2);
  }
  ...
  */
  const PauliSpinor& Khi(halfint spin) {
    ...
    if (spin == -half) { return Khi_minus; }
    if (spin == half) { return Khi_plus; }
  ...
  /// Adjoint
  const PauliSpinor AdPauliSpinor::adj() const {
    return PauliSpinor(conj(x1),conj(x2));
  }
  const PauliSpinor adj(const AdPauliSpinor& a) {
    return PauliSpinor(conj(a.x1),conj(a.x2));
  }
  /// Positive sign.
  const AdPauliSpinor& AdPauliSpinor::operator + () const {
    return *this;
  }
  /// Negative sign.
  const AdPauliSpinor AdPauliSpinor::operator - () const {
  ...
  /// Spinor addition.
  const AdPauliSpinor AdPauliSpinor::operator + (const AdPauliSpinor& a) const {
  ...
  /// Multiplication with scalar (dcomplex).
  const AdPauliSpinor AdPauliSpinor::operator * (const dcomplex& z) const {
  ...
  /// Friend operator for scalar*spinor.
  const AdPauliSpinor operator * (const dcomplex& z, const AdPauliSpinor& a) {
    return AdPauliSpinor(z*a.x1, z*a.x2);
  }
  /// Spinor*matrix product.
  const AdPauliSpinor operator * (const AdPauliSpinor& a, const PauliMatrix& tau) {
    return AdPauliSpinor(a.x1*tau.x11 + a.x2*tau.x21, a.x1*tau.x12 + a.x2*tau.x22);
  }
  /// Equality check.
  ...
  */
  ...
  /// Decide if any component is NaN.
  bool AdPauliSpinor::isnan() const {
    return ::isnan(x1) or ::isnan(x2);
  }
  ...
  /// Copy constructor.
  PauliMatrix::PauliMatrix(const PauliMatrix& _t) : x11(_t.x11),x12(_t.x12),x21(_t.x21),x22(_t.x22) {}
  /// Create a Pauli matrix from components.
  PauliMatrix::PauliMatrix(dcomplex x11, dcomplex x12, dcomplex x21, dcomplex x22) : x11(x11),x12(x12),x21(x21),x22(x22) {
    /*
  ...
  /// Adjoint
  const PauliMatrix PauliMatrix::adj() const {
    return PauliMatrix(std::conj(x11),std::conj(x21),std::conj(x12),std::conj(x22));
  }
  const PauliMatrix adj(const PauliMatrix& _t) {
    return PauliMatrix(std::conj(_t.x11),std::conj(_t.x21),std::conj(_t.x12),std::conj(_t.x22));
  }
  /// Positive sign.
  const PauliMatrix& PauliMatrix::operator + () const {
    return *this;
  }
  /// Negative sign.
  const PauliMatrix PauliMatrix::operator - () const {
  ...
  /// Matrix addition.
  const PauliMatrix PauliMatrix::operator + (const PauliMatrix& _t) const {
  ...
  /// Matrix product.
  const PauliMatrix operator * (const PauliMatrix& _t1, const PauliMatrix& _t2) {
  ...
  /// Multiplication with scalar (dcomplex).
  const PauliMatrix PauliMatrix::operator * (const dcomplex& z) const {
    return PauliMatrix(x11*z, x12*z, x21*z, x22*z);
  }
  /// Division by scalar.
  const PauliMatrix PauliMatrix::operator / (dcomplex z) const {
  ...
  const PauliMatrix operator * (const dcomplex& z, const PauliMatrix& _t) {
  ...
  void PauliMatrix::output(ostream& out, int fieldWidth) const {
    out << "( " << setw(fieldWidth) << x11 << "  " << setw(fieldWidth) << x12 << " )" << endl;
    out << "( " << setw(fieldWidth) << x21 << "  " << setw(fieldWidth) << x22 << " )" << endl;
  }
  /// Trace.
  dcomplex PauliMatrix::trace() const {
    return x11 + x22;
  }

  const PauliMatrix PauliMatrix::conj() const {
    return PauliMatrix(std::conj(x11),std::conj(x12),std::conj(x21),std::conj(x22));
  }
  const PauliMatrix PauliMatrix::transpose() const {
  ...
  /// Positive sign.
  const DiracSpinor& DiracSpinor::operator + () const {
    return *this;
  }
  /// Negative sign.
  const DiracSpinor DiracSpinor::operator - () const {
  ...
  /// Spinor addition.
  const DiracSpinor DiracSpinor::operator + (const DiracSpinor& u) const {
    return DiracSpinor(x[0]+u.x[0], x[1]+u.x[1], x[2]+u.x[2], x[3]+u.x[3]);
  }
  /// Spinor subtraction.
  const DiracSpinor DiracSpinor::operator - (const DiracSpinor& u) const {
    return DiracSpinor(x[0]-u.x[0], x[1]-u.x[1], x[2]-u.x[2], x[3]-u.x[3]);
  ...
  /// Multiplication with scalar (dcomplex).
  const DiracSpinor DiracSpinor::operator * (const dcomplex& z) const {
  ...
  /// Friend operator for scalar*spinor.
  const DiracSpinor operator * (const dcomplex& z, const DiracSpinor& u) {
    return DiracSpinor(u.x[0]*z, u.x[1]*z, u.x[2]*z, u.x[3]*z);
  }
  /// Matrix*spinor product.
  const DiracSpinor operator * (const DiracMatrix& gamma, const DiracSpinor& u) {
  ...
  /// Scalar product.
  const dcomplex operator * (const AdDiracSpinor& ubar, const DiracSpinor& u) {
    dcomplex z(0);
    for (int i(0); i<4; i++) {
      z += ubar.x[i] * u.x[i];
    }
  ...
  /// Diadic product.
  const DiracMatrix operator * (const DiracSpinor& u, const AdDiracSpinor& ubar) {
    DiracMatrix gamma;
    for (int i(0); i<4; i++) {
      for (int j(0); j<4; j++) {
  ...
  const AdDiracSpinor DiracSpinor::adj() const {
    return AdDiracSpinor(conj(x[0]),conj(x[1]),conj(x[2]),conj(x[3]))*gamma0_;
  }

  const AdDiracSpinor adj(const DiracSpinor& u) {
    return AdDiracSpinor(conj(u.x[0]),conj(u.x[1]),conj(u.x[2]),conj(u.x[3]))*gamma0_;
  }
  ...
  /// Create a spinor with the given components.
  AdDiracSpinor::AdDiracSpinor(dcomplex x0, dcomplex x1, dcomplex x2, dcomplex x3) {
    x[0] = x0;
    x[1] = x1;
    x[2] = x2;
    x[3] = x3;
  ...
  /// Positive sign.
  const AdDiracSpinor& AdDiracSpinor::operator + () const {
    return *this;
  }
  /// Negative sign.
  const AdDiracSpinor AdDiracSpinor::operator - () const {
  ...
  /// Spinor addition.
  const AdDiracSpinor AdDiracSpinor::operator + (const AdDiracSpinor& u) const {
    return AdDiracSpinor(x[0]+u.x[0], x[1]+u.x[1], x[2]+u.x[2], x[3]+u.x[3]);
  }
  /// Spinor subtraction.
  const AdDiracSpinor AdDiracSpinor::operator - (const AdDiracSpinor& u) const {
    return AdDiracSpinor(x[0]-u.x[0], x[1]-u.x[1], x[2]-u.x[2], x[3]-u.x[3]);
  ...
  /// Multiplication with scalar (dcomplex).
  const AdDiracSpinor AdDiracSpinor::operator * (const dcomplex& z) const {
  ...
  /// Friend operator for scalar*spinor.
  const AdDiracSpinor operator * (const dcomplex& z, const AdDiracSpinor& u) {
    return AdDiracSpinor(u.x[0]*z, u.x[1]*z, u.x[2]*z, u.x[3]*z);
  }
  /// spinor*matrix product.
  const AdDiracSpinor operator * (const AdDiracSpinor& u, const DiracMatrix& gamma) {
    //cerr << "AdDiracSpinor operator * (const AdDiracSpinor& u, const DiracMatrix& gamma)" << endl;
  ...
  const DiracSpinor AdDiracSpinor::adj() const {
    return gamma0_ * DiracSpinor(conj(x[0]), conj(x[1]), conj(x[2]), conj(x[3]));
  }

  const DiracSpinor adj(const AdDiracSpinor& ubar) {
    return gamma0_ * DiracSpinor(conj(ubar.x[0]), conj(ubar.x[1]), conj(ubar.x[2]), conj(ubar.x[3]));
  }
  ...
  DiracMatrix::DiracMatrix(dcomplex g00, dcomplex g01, dcomplex g02, dcomplex g03,
                           ...
                           dcomplex g30, dcomplex g31, dcomplex g32, dcomplex g33) {
    x[0][0] = g00;
    x[0][1] = g01;
    x[0][2] = g02;
    x[0][3] = g03;
  ...
  DiracMatrix::DiracMatrix(PauliMatrix t1, PauliMatrix t2, PauliMatrix t3, PauliMatrix t4) {
    x[0][0] = t1.x11;
    x[0][1] = t1.x12;
    x[0][2] = t2.x11;
    x[0][3] = t2.x12;
  ...
  /// Adjoint
  const DiracMatrix DiracMatrix::adj() const {
    return gamma0_ * (*this).trans_conj() * gamma0_;
  }
  const DiracMatrix adj(const DiracMatrix& g) {
    return gamma0_ * g.trans_conj() * gamma0_;
  }
  /// Positive sign.
  const DiracMatrix& DiracMatrix::operator + () const {
    return *this;
  }
  /// Negative sign.
  const DiracMatrix DiracMatrix::operator - () const {
  ...
  /// Matrix addition.
  const DiracMatrix DiracMatrix::operator + (const DiracMatrix& g) const {
    DiracMatrix gg;
    for (int i(0); i<4; i++) {
      for (int j(0); j<4; j++) {
  ...
  /// Matrix product.
  const DiracMatrix operator * (const DiracMatrix& g1, const DiracMatrix& g2) {
    DiracMatrix g;
    for (int i(0); i<4; i++) {
      for (int j(0); j<4; j++) {
        for (int k(0); k<4; k++) {
  ...
  /// Multiplication with scalar (dcomplex).
  const DiracMatrix DiracMatrix::operator * (const dcomplex& z) const {
    DiracMatrix g;
    for (int i(0); i<4; i++) {
      for (int j(0); j<4; j++) {
        g.x[i][j] = x[i][j] * z;
  ...
  /// Friend operator for scalar*matrix.
  const DiracMatrix operator * (const dcomplex& z, const DiracMatrix& g) {
    DiracMatrix gg;
    for (int i(0); i<4; i++) {
      for (int j(0); j<4; j++) {
        gg.x[i][j] = z * g.x[i][j];
  ...
  ostream& operator << (ostream& out, const DiracMatrix& g) {
    out << "("
        << "(" << g.x[0][0] << ", " << g.x[0][1] << ", " << g.x[0][2] << ", " << g.x[0][3] << "), "
        << "(" << g.x[1][0] << ", " << g.x[1][1] << ", " << g.x[1][2] << ", " << g.x[1][3] << "), "
        << "(" << g.x[2][0] << ", " << g.x[2][1] << ", " << g.x[2][2] << ", " << g.x[2][3] << "), "
        << "(" << g.x[3][0] << ", " << g.x[3][1] << ", " << g.x[3][2] << ", " << g.x[3][3] << ")"
        << ")";
    return out;
  }
  /// Print to a stream in the form of a table.
  void DiracMatrix::output(ostream& out, int fieldWidth) const {
    out << "(" << setw(fieldWidth) << x[0][0] << "   " << setw(fieldWidth) << x[0][1] << "   "
        ...
        << setw(fieldWidth) << x[3][2] << "   " << setw(fieldWidth) << x[3][3] << ")\n";
  }
  /// Trace.
  ...
  const DiracMatrix DiracMatrix::conj() const {
    DiracMatrix g;
    for (int i(0); i<4; i++) {
      for (int j(0); j<4; j++) {
  ...
  const DiracMatrix DiracMatrix::transpose() const {
    DiracMatrix g;
    for (int i(0); i<4; i++) {
      for (int j(0); j<4; j++) {
        g.x[i][j] = x[j][i];
  ...
  const DiracMatrix DiracMatrix::trans_conj() const {
    DiracMatrix g;
    for (int i(0); i<4; i++) {
      for (int j(0); j<4; j++) {
  ...
  const Gamma_mu& Gamma_mu::instance() {
    return gamma_;
  }
  
  const DiracMatrix Gamma_mu::operator()(int mu) const {
    assert(0<=mu and mu<4);
    if (mu == 0) { return gamma0_; }
    if (mu == 1) { return -gamma1_; }
    if (mu == 2) { return -gamma2_; }
    if (mu == 3) { return -gamma3_; }
  ...
  const DiracMatrix Gamma_mu::operator()(const FourVector& p) const {
  ...
  const Sigma_mu_nu& Sigma_mu_nu::instance() {
    return sigma_;
  }

  const DiracMatrix Sigma_mu_nu::operator()(int mu, int nu) const {
    assert(0<=mu and mu<4);
    assert(0<=nu and nu<4);
    return i_/2. * (gamma_(mu)*gamma_(nu) - gamma_(nu)*gamma_(mu));
  }
  const DiracMatrix Sigma_mu_nu::operator()(int mu, const FourVector& p) const {
    assert(0<=mu and mu<4);
    DiracMatrix ret = gamma_null;
    for (int nu(0); nu<4; nu++) {
      ret += i_/2. * (gamma_(mu)*gamma_(nu) - gamma_(nu)*gamma_(mu)) * p[nu];
    }
    return ret;
  }

  const DiracMatrix Sigma_mu_nu::operator()(const FourVector& p, int nu) const {
    assert(0<=nu and nu<4);
    DiracMatrix ret = gamma_null;
    for (int mu(0); mu<4; mu++) {
      ret += i_/2. * (gamma_(mu)*gamma_(nu) - gamma_(nu)*gamma_(mu)) * p[mu];
    }
    return ret;
  }

  const DiracMatrix Sigma_mu_nu::operator()(const FourVector& p1, const FourVector& p2) const {
    DiracMatrix ret = gamma_null;
    for (int mu(0); mu<4; mu++) {
      for (int nu(0); nu<4; nu++) {
        ret += i_/2. * (gamma_(mu)*gamma_(nu) - gamma_(nu)*gamma_(mu)) * p1[mu] * p2[nu];
      }
    }
    return ret;
  }
