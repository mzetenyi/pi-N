...
namespace Vectors {
  ...
  const double& ThreeVector::operator [] (int i) const {
    assert(isIndex3(i));
    return x[i];
  }
  ...
  */
  ...
  double& ThreeTensor::operator () (int i,int j) {
    assert(isIndex3(i));
    assert(isIndex3(j));
    return x[i][j];
  }

  const double& ThreeTensor::operator () (int i,int j) const {
    assert(isIndex3(i));
    assert(isIndex3(j));
    return x[i][j];
  }
  ...
  const ThreeVector ThreeTensor::operator * (const ThreeVector& v) const {
    return ThreeVector(x[1][1]*v[1] + x[1][2]*v[2] + x[1][3]*v[3],
                       x[2][1]*v[1] + x[2][2]*v[2] + x[2][3]*v[3],
                       x[3][1]*v[1] + x[3][2]*v[2] + x[3][3]*v[3]);
  }

  const ThreeVector operator * (const ThreeVector& v, const ThreeTensor& t) {
    return ThreeVector(v[1]*t.x[1][1] + v[2]*t.x[2][1] + v[3]*t.x[3][1],
                       v[1]*t.x[1][2] + v[2]*t.x[2][2] + v[3]*t.x[3][2],
                       v[1]*t.x[1][3] + v[2]*t.x[2][3] + v[3]*t.x[3][3]);
  }

  const ThreeTensor diad(const ThreeVector& v, const ThreeVector& w) {
    return ThreeTensor(v[1]*w[1],v[1]*w[2],v[1]*w[3],
                       v[2]*w[1],v[2]*w[2],v[2]*w[3],
                       v[3]*w[1],v[3]*w[2],v[3]*w[3]);
  }
  ...
  void ThreeTensor::output(ostream& out, int fieldWidth) const {
  ...
  ThreeTensor ThreeTensor::diagonalTensor(const ThreeVector& v) {
    return ThreeTensor(v[1],v[2],v[3]);
  }
  ...
  double& FourVector::operator [] (int mu) {
    assert(isIndex4(mu));
    return x[mu];
  }

  const double& FourVector::operator [] (int mu) const {
    assert(isIndex4(mu));
    return x[mu];
  }
  ...
  double FourTensor::operator () (int mu,int nu) const {
    assert(isIndex4(mu));
    assert(isIndex4(nu));
    return sign_(mu) * x[mu][nu];
  }

  const FourVector FourTensor::operator [] (int mu) const {
    assert(isIndex4(mu));
    ...
    for (int mu(0); mu<4; mu++) {
      for (int nu(0); nu<4; nu++) {
        for (int ro(0); ro<4; ro++) {
          ret.x[mu][nu] += x[mu][ro] * other.x[ro][nu];
        }
      }
    }
    return ret;
  ...
  const FourVector operator * (const FourVector& v, const FourTensor& t) {
    ...
    for (int mu(0); mu<4; mu++) {
      for (int nu(0); nu<4; nu++) {
        ret.x[mu] += sign_(mu)*sign_(nu) * v.x[nu] * t.x[nu][mu];
      }
    }
    return ret;
  }
  ...
  const FourTensor diad(const FourVector& v, const FourVector& w) {
    ...
    for (int mu(0); mu<4; mu++) {
      for (int nu(0); nu<4; nu++) {
        ret.x[mu][nu] = sign_(nu) * v.x[mu] * w.x[nu];
      }
    }
    return ret;
  }
  ...
  void FourTensor::output(ostream& out, int fieldWidth) const {
