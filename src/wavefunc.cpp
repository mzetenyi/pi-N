...
eps_::eps_(FourVector p) : val(idx_lor,idx_s1) {
  ...
  if (m2/(p[0]*p[0]) < 0.000001) { // zero mass particle -- in radiation gauge, i.e. e_[0]=0
    const ThreeVector pp = p.spacial();
    const ThreeVector ex(1,0,0);
    const ThreeVector ey(0,1,0);
    ThreeVector e1 = ((ex*pp < sqrt(pp*pp/2)) ? crossprod(pp,ex) : crossprod(pp,ey));
    e1 /= sqrt(e1*e1);
    ...
    //const FourVector eps3 = (p - (p*eps0)*eps0)/sqrt((p*eps0)*(p*eps0) - p*p);
...
u_1h::u_1h(FourVector p) : val(idx_s1h) {
  init(p);
}
...
ubar_1h::ubar_1h(FourVector p) : val(idx_s1h) {
  init(p);
}
...
v_1h::v_1h(FourVector p) : val(idx_s1h) {
  init(p);
}
...
u_3h::u_3h(FourVector p) : val(idx_lor,idx_s3h) {
  init(p);
}
...
void u_3h::init(FourVector p) {
  ...
  if (m2 < 0) {
    if (-m2/(p[0]*p[0]) > 0.000001) {
      cerr << "spacelike fourmomentum in u_(int mu, FourVector p, int s): " << p << endl;
      exit(0);
    }
    m2 = 0;
  }
  if (m2/(p[0]*p[0]) < 0.000001) { // zero mass particle
    cerr << "u_(int mu, FourVector p, int s): zero mass particle - not implemented." << endl;
    exit(0);
  }
  u_1h u_(p);
  eps_ e_(p);
  for (halfint mu(_0); mu<_4; mu++) {
    ...
    val(mu,half) = 1./sqrt(3.)*e_(mu,_1)*u_(-half) + sqrt(2./3.)*e_(mu,_0)*u_(half);
    val(mu,-half) = sqrt(2./3.)*e_(mu,_0)*u_(-half) + 1./sqrt(3.)*e_(mu,-_1)*u_(half);
    val(mu,-3*half) = e_(mu,-_1)*u_(-half);
  }
}
...
ubar_3h::ubar_3h(FourVector p) : val(idx_lor,idx_s3h) {
  init(p);
}
...
u_5h::u_5h(FourVector p) : val(idx_lor,idx_lor,idx_s5h) {
  init(p);
}
...
void u_5h::init(FourVector p) {
  ...
  if (m2 < 0) {
    if (-m2/(p[0]*p[0]) > 0.000001) {
      cerr << "spacelike fourmomentum in u_(int mu, int nu, FourVector p, int s): " << p << endl;
      exit(0);
    }
    m2 = 0;
  }
  if (m2/(p[0]*p[0]) < 0.000001) { // zero mass particle
    cerr << "u_(int mu, int nu, FourVector p, int s): zero mass particle - not implemented." << endl;
    exit(0);
  }
  u_3h u_(p);
  eps_ e_(p);
...
ubar_5h::ubar_5h(FourVector p) : val(idx_lor,idx_lor,idx_s5h) {
  init(p);
}
...
u_::u_(Particle part) : J(part.spin()), u_1hp(NULL), u_3hp(NULL), u_5hp(NULL) {
  ...
  if (J==half) {
    ...
    cerr << "u_(Particle):  invalid or unimplemented spin of baryon, J = " << J << endl;
    exit(0);
  }
}
*/
...
*/
u_::~u_() {
  if (u_1hp) { delete u_1hp; }
  if (u_3hp) { delete u_3hp; }
  if (u_5hp) { delete u_5hp; }
}
/** 
...
/**
   ...
   @param p four-momentum
*/
ubar_::ubar_(halfint J, FourVector p) : J(J), ubar_1hp(NULL), ubar_3hp(NULL), ubar_5hp(NULL) {
  assert(p.future());
  if (J==half) {
...
*/
/*
ubar_::ubar_(Particle part) : J(part.spin()), ubar_1hp(NULL), ubar_3hp(NULL), ubar_5hp(NULL) {
  assert(part.fourmom().future());
  if (J==half) {
    ubar_1hp = new ubar_1h(part.fourmom());
  } else if (J==3*half) {
    ubar_3hp = new ubar_3h(part.fourmom());
  } else if (J==5*half) {
    ubar_5hp = new ubar_5h(part.fourmom());
  } else {
    cerr << "ubar_(Particle):  invalid or unimplemented spin of baryon, J = " << J << endl;
    exit(0);
  }
}
*/
...
*/
ubar_::~ubar_() {
  if (ubar_1hp) { delete ubar_1hp; }
  if (ubar_3hp) { delete ubar_3hp; }
  if (ubar_5hp) { delete ubar_5hp; }
}
/** 
    Return the components of the spinor amplitude.
    @param mu "cumulated" Lorentz indices: mu = mu1 + 4*(mu2 + 4*(mu3 + ...) ).
...
pro5h::pro5h(Particle par) : val(idx_lor,idx_lor,idx_lor,idx_lor) {
  FourVector p = par.fourmom();
  double m = par.mass();
  Array<double,2> G(idx_lor,idx_lor);
  Array<DiracMatrix,2> T(idx_lor,idx_lor);
  for (int mu(0); mu<4; mu++) {
    for (int nu(0); nu<4; nu++) {
    ...
    for (int mu2(0); mu2<4; mu2++) {
      for (int nu1(0); nu1<4; nu1++) {
        for (int nu2(0); nu2<4; nu2++) {
          val(mu1,mu2,nu1,nu2) +=
            - 3./10. * (G(mu1,nu1)*G(mu2,nu2) + G(mu1,nu2)*G(mu2,nu1)) * gamma_unit
            + 1./5. * G(mu1,mu2)*G(nu1,nu2) * gamma_unit
            + 1./10. * (T(mu1,nu1)*G(mu2,nu2) + T(mu2,nu2)*G(mu1,nu1) + T(mu1,nu2)*G(mu2,nu1) + T(mu2,nu1)*G(mu1,nu2));
        }
      }
    }
  }
...
DiracMatrix pro5h::_T(double m, int mu, int nu, FourVector p) {
  return -1./2. * (gamma_[mu]*gamma_[nu] - gamma_[nu]*gamma_[mu])
    + 1./(2.*m) * p[mu]*(gamma_(p)*gamma_[nu] - gamma_[nu]*gamma_(p))
    - 1./(2.*m) * p[nu]*(gamma_(p)*gamma_[mu] - gamma_[mu]*gamma_(p));
}
